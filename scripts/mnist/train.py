...
def obtain_batch(size=BATCH_SIZE) -> tuple[ArrayLike, ArrayLike]:
    ...
    # index = jax.random.randint(key, (0,), 0, len(dataset.data) - size + 1)
    return dataset.data[index:index + size], dataset.target[index:index + size]

def infer(parameters: ArrayLike, input_: ArrayLike, log: bool = False) -> ArrayLike:
    no_inputs = 8 * 8
    no_hidden = 16
    no_classes = 10
    ...
    model = Model([Dense(W1, b1),
                   ReLU(),
                   Dense(W2, b2),
                   Softmax(log)])

    return model.infer(input_)
...
def evaluate(parameters: ArrayLike, input_: ArrayLike, class_: int) -> float:
    ...
    actual_output = infer(parameters, input_, log=True)
